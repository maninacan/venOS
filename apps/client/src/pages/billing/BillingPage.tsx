...
export function BillingPage() {
  ...
  useEffect(() => {
    ...
    if (!billing) return;
    if (billing === 'success') {
      showToast(t('toast.paymentReceived', 'Payment received — activating Venview Pro…'), 'success', 5000);
    ...
    <>
      <div className="card">
        <div className="mb-4">
          ...
          <p className="text-[#64748b] text-[0.86rem] m-0">{t('subtitle', 'Manage your Venview subscription and payment details.')}</p>
        </div>
        ...
        {!loading && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            <span className={`inline-flex items-center gap-1.5 text-[0.9rem] font-semibold px-[14px] py-1 rounded-full ${isPro ? 'bg-[#fef3c7] text-[#92400e]' : 'bg-[#f1f5f9] text-[#64748b]'}`}>
              {isPro ? <><i className="fa-solid fa-bolt" /> {t('planPro', 'Venview Pro')}</> : <><i className="fa-solid fa-clipboard-list" /> {t('planStarter', 'Venview Starter')}</>}
            </span>
      ...
      {!loading && !isPro && (
        <div className="card">
          <p style={{ fontSize: '0.78rem', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 12px' }}>{t('upgrade.eyebrow', 'Upgrade to Venview Pro')}</p>
